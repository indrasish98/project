...
import {
    addressSchema
} from "../../validation/project/validation.address.js";
import {
    amenitySchema
} from "../../validation/project/validation.amenity.js";
import {
    employeeSchema
} from "../../validation/project/validation.employee.js";
import {
    paymentPlanSchema
} from "../../validation/project/validation.paymentPlan.js";
import planNameSchema from "../../validation/project/validation.planName.js";
import {
    projectSchema
} from "../../validation/project/validation.project.js";
import {
    projectDetailSchema
} from "../../validation/project/validation.projectDetails.js";
import {
    roomConfigurationSchema
} from "../../validation/project/validation.roomConfiguration.js";
...
export const projectCreationValidation = async (req, res, next) => {
    ...
    if (projectDetail) {
        const {
            error
        } = projectDetailSchema.validate(projectDetail);
        if (error) {
            const errorMessage = error.details[0].message;
            const message = makeClearMessage(errorMessage);
            return next(new CustomError(message, 400));
    ...
    if (planName && planName.length > 0) {
        ...
        for (const element of planName) {
            ...
            const {
                error
            } = planNameSchema.validate(element);
    ...
    if (roomConfiguration && roomConfiguration.length > 0) {
        ...
        for (const element of roomConfiguration) {
            ...
            const {
                error
            } = roomConfigurationSchema.validate(element);
    ...
    if (siteEmployee && siteEmployee.length > 0) {
        ...
        for (const element of siteEmployee) {
            ...
            const {
                error
            } = employeeSchema.validate(element);
    ...
    if (amenity) {
        ...
        const {
            error
        } = amenitySchema.validate(amenity);
...
export const roomConfigurationValidation = async (req, res, next) => {
    ...
    const {
        error
    } = roomConfigurationSchema.validate(req.body);
...
export const addressValidation = async (req, res, next) => {
    ...
    const {
        error
    } = addressSchema.validate(req.body);
...
export const paymentPlanValidation = async (req, res, next) => {
    ...
    const {
        error
    } = paymentPlanSchema.validate(req.body);
...
export const amenityValidation = async (req, res, next) => {
    ...
    const {
        error
    } = amenitySchema.validate(req.body);
...
export const planNameValidation = async (req, res, next) => {
    ...
    const {
        error
    } = planNameSchema.validate(req.body);
